...
#define  _FIGURE_NAME_ "fig_distmt"
#define  _N_HISTOGRAMS_ 3

void figure_distmt(TString aEventDir = "./events/", Int_t aEventFiles = 1)
{
// ##########################################################################
// # READ ROOT FILES
// ##########################################################################
...
// Create histograms
  ...
  H1D[0]->GetXaxis()->SetTitle("m_{T}-m_{0} [GeV/c]");
  H1D[0]->GetYaxis()->SetTitle("dN/(2 #pi m_{T} dm_{T} dy)");
  H1D[0]->Sumw2();
...
// Fill histograms
  ...
  for(Int_t i=0; i<Chain->GetEntries(); i++) {
    ...
    if( TMath::Abs(Rap) >= 1.0 )
      continue;
    pid = Particle.pid;
    Mt  = TMath::Sqrt(Particle.mass*Particle.mass + Particle.px*Particle.px + Particle.py*Particle.py);
    if(pid == 211)
      H1D[0]->Fill(Mt-Particle.mass, 1.0/Mt);
    if(pid == 321)
      H1D[1]->Fill(Mt-Particle.mass, 1.0/Mt);
    if((pid == 2212) && (Particle.fatherpid != 3122)) // protons without the ones form (Lm1115zer)
      H1D[2]->Fill(Mt-Particle.mass, 1.0/Mt);
  }
// Rescale histograms
