...
void CoulombAfterburner::ReadParameters()
{
    ...
    tMainConfig = new Configurator;
    tParser = new Parser(sMainINI.Data());
    tParser->ReadINI(tMainConfig);
    delete tParser;

    try
    {
        m_nSteps = tMainConfig->GetParameter("CoulombTimeSteps").Atoi();
        m_StepSize = tMainConfig->GetParameter("CoulombStepSize").Atof() / kHbarC;
    }
    catch (TString &str)
    {
                ...
                for (; tPartOtherIter != tParticles->end(); ++tPartOtherIter)
                {
                    ...
                    else
                    {
                        ...
                        if (tBestI >= 0 && tBestI < tCoordinates[tPartOtherIter->eid].size())
                        {
                            ...
                            {
                                cout << endl
                                     << "Step " << iStep << ", Condition: " << (tBestInterval < m_StepSize) << " " << tBestInterval << " " << tBestI << " " << tCoordinates[tPartOtherIter->eid].size() << " " << tPartOtherIter->pid << " " << tPartOtherIter->eid << " " << tParticles->size() << " " << tEvent->GetEventID() << endl;
                                cout << Rdotu << " "
                                     << " " << v.X() << " " << v.Y() << " " << v.Z() << endl;
