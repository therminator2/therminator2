...
Model_SR::Model_SR(): Model(),mThermo(0),mR(0.0),mH(0.0),mA(0.0),mT0(0.0),mGammaS(0.0)
{}

Model_SR::Model_SR(TRandom2* aRandom):Model(aRandom) 
{
    mName = "SR";
    mThermo = new Thermodynamics();
    ReadParameters();
    Description();
    mHyperCube = mR * 2*Pi()*Pi() * 2*Pi()*Pi();
}
...
double Model_SR::GetIntegrand(ParticleType* aPartType, bool finiteWidth)
{
    ...
    double fugacity = Power(mThermo->GetGammaQ(),  aPartType->GetNumberQ() + aPartType->GetNumberAQ()) * Power(mThermo->GetGammaS(),  aPartType->GetNumberS() + aPartType->GetNumberAS()) * Exp(mThermo->GetChemicalPotential(aPartType) / Temp);
...
void Model_SR::ReadParameters()
{
    ...
    try 
    {
        ...
        {
            mThermo->SetChemistry(tModelParam->GetParameter("MuB").Atof() * 0.001,tModelParam->GetParameter("MuI").Atof() * 0.001,tModelParam->GetParameter("MuS").Atof() * 0.001,tModelParam->GetParameter("MuC").Atof() * 0.001);	// [GeV]
            mThermo->SetGammas(tModelParam->GetParameter("GammaQ").Atof(),tModelParam->GetParameter("GammaS").Atof(),tModelParam->GetParameter("GammaC").Atof()); // [1]
        }
        else if(tModelParam->GetParameter("Chemistry") == "gamma_lambda")
        {
            mThermo->SetChemistry(tModelParam->GetParameter("LambdaQ").Atof(),tModelParam->GetParameter("LambdaI").Atof(),tModelParam->GetParameter("LambdaS").Atof(),tModelParam->GetParameter("LambdaC").Atof(),tModelParam->GetParameter("GammaQ").Atof(),tModelParam->GetParameter("GammaS").Atof(),tModelParam->GetParameter("GammaC").Atof()); // [1]
        ...
        if(mH < tVectorR->GetXMin() || mH > tVectorR->GetXMax() || mEps < tVectorR->GetYMin() || mEps > tVectorR->GetYMax() || mDel < tVectorR->GetZMin() || mDel > tVectorR->GetZMax()) 
        {
            cout << "The values of (H, epsilon, delta) = (" << mH << "," << mEps << "," << mDel << ") outside of the grid (" << tVectorR->GetXMin() << ":" << tVectorR->GetXMax() << "," << tVectorR->GetYMin() << ":" << tVectorR->GetYMax() << "," << tVectorR->GetZMin() << ":" << tVectorR->GetZMax() << ")" << endl;
            exit(_ERROR_CONFIG_PARAMETER_NOT_FOUND_);
        }
