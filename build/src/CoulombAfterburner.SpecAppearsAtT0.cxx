...
CoulombAfterburner::CoulombAfterburner(int aSteps, double aStepSize, Double_t aR) : m_nSteps(aSteps), m_StepSize(aStepSize / kHbarC), mR(aR)
{
    ...
    m_gImpBNPart = new TGraph(200, xs, ys);
}
...
ParticleType *CoulombAfterburner::AddSpectators(std::list<Particle> *tParticles, Float_t tEarliestTime)
{
    ...
    Float_t tSpectatorZ = 0;//tSpectatorBeta * tEarliestTime;
    // Linear interpolation between:
    // tNpart = 2*tA -> tSpectatorX = tR
    // tNpart = 0 -> tSpectatorX = tImpB
    Float_t tSpectatorX = mR * tNpart / (2 * tA) + tImpB / (2 * tA) * (2 * tA - tNpart);
    ...
    tSpectator1.SetParticlePX(tSpectatorE, 0, 0, tSpectatorPz, 0.0, tSpectatorX, 0.0, tSpectatorZ);
    tParticles->push_back(tSpectator1);

    Particle tSpectator2(tSpectatorType);
    tSpectator2.SetParticlePX(tSpectatorE, 0, 0, -tSpectatorPz, 0.0, -tSpectatorX, 0.0, -tSpectatorZ);
