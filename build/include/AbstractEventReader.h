...
class AbstractEventReader {
  public:
    AbstractEventReader();
    AbstractEventReader(ParticleDB* aDB, AbstractEventSaver *aES, ListAfterburner *aLA);
    ~AbstractEventReader();

    void   ReadEvents();

  protected:
    ParticleCoor mParticleCoor;
    StructEvent mStructEvent;
    int		        mNumberOfEvents;
    int                 mEventIter;

    virtual void SetupInput(TString aEventDir, Int_t aEventFiles) = 0;
    virtual bool NextEvent() = 0;
    virtual void GetParticle(int aIter) = 0;
