...
submissiondir="/lustre/hades/user/harabasz/submit/HubbDeltCaseB_0steps10_fixedPositionUpdate_urqmd_eosFlag1_1"
outputdir="/lustre/hades/user/harabasz/HubbDeltCaseB_0steps10_fixedPositionUpdate_urqmd_eosFlag1_1"
        ...
        for delta in deltalist:
            for i,prime in enumerate(mygeneraotr):
                ...
                try:
                    ...
                    copy("Au_Au_1230MeV_bmax10_f15.input",dir)
                    copy("tables.dat",dir)
...
copy("jobScript_urqmd_SL.sh", submissiondir)
...
command="sbatch --array=1-{3} --mem=2000 --time=0-48:00:00 --partition=long -D {0}  --output={1}/out/slurm-%A_%a.out -- {0}/jobScript_urqmd_SL.sh {0}/jobarray.dat {1} {2}".format(submissiondir,outputdir,scriptdir,arrElem+1)
print(command)
os.system(command)
